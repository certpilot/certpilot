...
| Policy engine | ⚠️ | `key_size`, `key_type`, `max_lifetime`, `ca_restriction`, `naming`; a rule type it cannot evaluate is a violation, not a pass. **It runs on one of the three issuance paths.** A person requesting a certificate is judged by it — through the issuance resolver, which applies the floor after the template has had its say; an agent requesting one is judged only by its grant, and a renewal is judged by nothing at all |
| Certificate templates | ⚠️ | What a kind of certificate looks like, with the three dispositions a policy cannot express — supply a value, constrain it, or pass it through. Enforced on **a person's request**: key type and size, curve, naming, wildcards, lifetime, custody, required metadata, and a supplied subject a CSR may not contradict. A template that could never issue anything is refused when it is saved. **The agent path and the renewal sweep do not consult one yet** |
