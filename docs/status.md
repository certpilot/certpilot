...
| Policy engine | ⚠️ | `key_size`, `key_type`, `max_lifetime`, `ca_restriction`, `naming`; a rule type it cannot evaluate is a violation, not a pass. **It runs on one of the three issuance paths.** A person requesting a certificate is judged by it; an agent requesting one is judged only by its grant, and a renewal is judged by nothing at all |
| Certificate templates | ⚠️ | The object exists: what a kind of certificate looks like, with the three dispositions a policy cannot express — supply a value, constrain it, or pass it through. A template that could never issue anything is refused when it is saved rather than when somebody needs a certificate. **Nothing reads one at issuance yet** |
