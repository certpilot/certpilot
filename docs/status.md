...
| Deployment profiles | ✅ | Nine platforms the agent installs to by name rather than by four hand-written fields: nginx, Apache, HAProxy, Caddy, Tomcat, PostgreSQL, MariaDB/MySQL, Postfix, Dovecot. Every one has been installed to in a container running the real service, with a TLS handshake proving it served the installed certificate after its own reload — `make verify-profiles` is that test, not a description of it. A profile is a default and any field an operator writes wins |
